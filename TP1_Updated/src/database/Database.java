 ...
 */
	private void createTables() throws SQLException {
	    ...
	    String onetimePasscodeTable = "CREATE TABLE IF NOT EXISTS PasswordReset ("
	    ...
	    statement.execute(onetimePasscodeTable);
	}
...
//		System.out.println(userList);
	...
	public boolean deleteUser(String userName) {
		...
		if (!doesUserExist(userName)) {
			System.err.println("User '" + userName + "' does not exist in the database.");
			return false;
		}
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			if (rowsAffected > 0) {
				System.out.println("User '" + userName + "' successfully deleted from database.");
				return true;
			} else {
				System.err.println("No rows affected when attempting to delete user '" + userName + "'.");
				return false;
			}
		} catch (SQLException e) {
 ...
 */
	 ...
	 * <p> Method: String generatePasswordResetOTP(String username) </p>

	 * 

	 * <p> Description: Generates a one-time passcode (OTP) for password recovery

	 * for the specified username. The OTP is stored in the PasswordReset table

	 * with a limited expiration time. </p>

	 * 

	 * @param username specifies the username requesting a password reset.

	 * 

	 * @return a six-character alphanumeric OTP used to verify the password reset request,
	...
	public String generatePasswordResetOTP(String username) {
	    ...
	    // Generate a 6-character alphanumeric OTP (no hyphens, easier to type)
	    ...
	    // First, delete any existing OTPs for this username to prevent duplicates

	    String deleteQuery = "DELETE FROM PasswordReset WHERE username = ?";
	    ...
	    String query = "INSERT INTO PasswordReset (username, otp, expiresAt) VALUES (?, ?, DATEADD('MINUTE', 10, CURRENT_TIMESTAMP))";
	    ...
	    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	        ...
	        pstmt.setString(2, otp.toString());
	 ...
	 * <p> Method: boolean verifyOTP(String username, String otp) </p>

	 * 

	 * <p> Description: Verifies that the provided one-time passcode (OTP)

	 * matches the stored OTP for the specified username and that it has

	 * not expired. </p>

	 * 

	 * @param username specifies the username requesting password reset verification.

	 * 

	 * @param otp specifies the one-time passcode entered by the user.

	 * 

	 * @return true if the OTP is valid and unexpired, else false.
	...
	public boolean verifyOTP(String username, String otp) {
	    ...
	    // Case-insensitive comparison to be user-friendly
	 ...
	 * <p> Method: void resetPassword(String username, String newPassword) </p>

	 * 

	 * <p> Description: Resets the password for the specified user by updating

	 * the stored password and removing any associated password reset OTP

	 * to prevent reuse. </p>

	 * 

	 * @param username specifies the username whose password is being reset.

	 * 

	 * @param newPassword specifies the new password to replace the old one.
	...
	public void resetPassword(String username, String newPassword) {
	    ...
	    String updateUser = "UPDATE userDB SET password = ? WHERE userName = ?";

	    String deleteOTP = "DELETE FROM PasswordReset WHERE username = ?";



	    try (

	        PreparedStatement p1 = connection.prepareStatement(updateUser);

	        PreparedStatement p2 = connection.prepareStatement(deleteOTP)
	...
	public void updatePassword(String userName, String newPassword) {
	    ...
	    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	        ...
	        int rowsAffected = pstmt.executeUpdate();
	        if (rowsAffected > 0) {
	            // Update the currentPassword if the username matches the currently logged-in user
